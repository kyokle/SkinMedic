    ...
    public function getAvailableTimes(Request $request)
{
    ...
    if ($scheduleStart !== null && $scheduleEnd !== null) {
        $allSlots = array_values(array_filter($allSlots, function($t) use ($scheduleStart, $scheduleEnd) {
    ...
    if ($preference === 'AM') {
        $allSlots = array_values(array_filter($allSlots, fn($t) => (int)explode(':', $t)[0] < 12));
    } elseif ($preference === 'PM') {
        $allSlots = array_values(array_filter($allSlots, fn($t) => (int)explode(':', $t)[0] >= 12));
    }

    // ── Filter past slots for today (PHT UTC+8) ──
    $nowManila   = now()->setTimezone('Asia/Manila');
    $todayManila = $nowManila->toDateString();
    if ($date === $todayManila) {
        $currentTime = $nowManila->format('H:i');
        $allSlots = array_values(array_filter($allSlots, fn($t) => $t > $currentTime));
    }
...
function parseScheduleTime(string $time, string $other = ''): int
{
    ...
    if (str_contains($time, 'PM') && $hour !== 12) {
        ...
        elseif ($hour <= 7) $hour += 12; // e.g. bare "7" → 19
    }
