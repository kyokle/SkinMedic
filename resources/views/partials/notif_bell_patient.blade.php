<div style="position:relative;">
    ...
    <div id="notifDropdown"
        ...
        <div style="padding:12px 16px;font-weight:600;font-size:14px;border-bottom:1px solid #eee;
            ...
            <button onclick="markAllPatientRead()"
        ...
        <div style="display:flex;border-bottom:1px solid #eee;background:#fafafa;overflow-x:auto;">
            @foreach(['all' => 'All', 'upcoming' => 'Upcoming', 'rescheduled' => 'Rescheduled', 'cancelled' => 'Cancelled', 'history' => 'History'] as $key => $label)
            <button onclick="switchPatientTab('{{ $key }}', this)"
                    data-tab="{{ $key }}"
                    style="flex-shrink:0;padding:10px 10px;font-size:11px;border:none;background:none;
...
function loadPatientUnreadCount() {
...
function toggleNotifDropdown() {
    ...
    if (!isOpen) loadPatientNotifications(currentPatientTab);
}

function switchPatientTab(type, btn) {
    currentPatientTab = type;
    ...
    loadPatientNotifications(type);
}

function loadPatientNotifications(type) {
    fetch('/notifications/by-type?type=' + type)
        .then(r => r.json())
        .then(items => {
            ...
            const icons = { upcoming: '🟢', rescheduled: '🔄', cancelled: '🔴', history: '✅', general: '🔔' };

            list.innerHTML = items.map(n => `
                <div onclick="handlePatientNotifClick(${n.id}, '${n.type}', ${n.reference_id || 0})"
...
function handlePatientNotifClick(id, type, referenceId) {
    fetch('/notifications/read', {
        ...
        if (referenceId) {
            window.location.href = '/patient/bookings?open=' + referenceId;
        } else {
            loadPatientUnreadCount();
            loadPatientNotifications(currentPatientTab);
        }
    });
}

function markAllPatientRead() {
    fetch('/notifications/read-all-by-type?type=' + currentPatientTab, {
        method: 'POST',
...
loadPatientUnreadCount();
setInterval(loadPatientUnreadCount, 30000);
</script>