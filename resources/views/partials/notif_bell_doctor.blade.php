<div style="position:relative;">
    ...
    <div id="notifDropdown"
        ...
        <div style="padding:10px 14px;font-weight:600;border-bottom:1px solid #eee;
            ...
            <span>👨‍⚕️ My Schedule</span>
            <button onclick="markAllDoctorRead()"
        ...
        <div style="display:flex;border-bottom:1px solid #eee;background:#fafafa;">
            @foreach(['all' => 'All', 'booking' => 'Today', 'rescheduled' => 'Rescheduled'] as $key => $label)
            <button onclick="switchDoctorTab('{{ $key }}', this)"
                    data-tab="{{ $key }}"
...
<script>
let currentDoctorTab = 'all';

function loadDoctorUnreadCount() {
...
function toggleNotifDropdown() {
    ...
    if (!isOpen) loadDoctorNotifications(currentDoctorTab);
}

function switchDoctorTab(type, btn) {
    currentDoctorTab = type;
    ...
    loadDoctorNotifications(type);
}

function loadDoctorNotifications(type) {
    fetch('/notifications/by-type?type=' + type)
        ...
        .then(items => {
            ...
            const icons = { booking: '📅', rescheduled: '🔄', cancelled: '❌', general: '🔔' };
            list.innerHTML = items.map(n => `
                <div onclick="handleDoctorNotifClick(${n.id}, '${n.type}', ${n.reference_id || 0})"
...
function handleDoctorNotifClick(id, type, referenceId) {
    fetch('/notifications/read', {
        ...
        if (referenceId) {
            window.location.href = '/doctor/bookings?open=' + referenceId;
        } else {
            loadDoctorUnreadCount();
            loadDoctorNotifications(currentDoctorTab);
        }
    });
}

function markAllDoctorRead() {
    fetch('/notifications/read-all-by-type?type=' + currentDoctorTab, {
        method: 'POST',
...
loadDoctorUnreadCount();
setInterval(loadDoctorUnreadCount, 30000);
</script>